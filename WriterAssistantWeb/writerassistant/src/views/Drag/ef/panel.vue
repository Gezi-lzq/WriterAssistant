        ...
        <div style="display: flex;height: calc(100% - 47px);">
            ...
            <div id="efContainer" ref="efContainer" class="container" v-flowDrag>
                <template v-for="node in data.nodeList">
                    <!-- 每一个node组件拿到的activeElement均是pane父组件所传过去的 -->
                    <flow-node
                            :id="node.id"
                            :key="node.id"
                            :node="node"
...
export default {
      ...
      bind (el, binding, vnode, oldNode) {
        ...
        el.onmousedown = (e) => {
          ...
          document.onmousemove = function (e) {
            // 移动时禁止默认事件(event.preventDefault() 方法阻止元素发生默认的行为。)
            e.preventDefault()
            // 移动
  ...
  methods: {
    ...
    // 更改activeElement的数据
      ...
      // 直接访问nodeForm子组件,根据当前激活Node展示Form
    ...
    repaintEverything () {
      console.info('repaintEverything')
    ...
    // // 模拟加载数据dataE，自适应创建坐标
    dataReloadE () {
      const dataE = getDataA()
      const tempData = lodash.cloneDeep(dataE)
